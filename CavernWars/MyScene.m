        ...
        if (unit.owner == 0) {
            ...
            if (unitcoor.y <= 23) {
                if (unitcoor.x >= 240) {
                    ...
                    //NSLog(@"%d" , unit.damage);
                    AIhealth = AIhealth - unit.damage;
                    [unit setHealth:(unit.health - 100)];
                }
                else {
                    [unit setPosition:CGPointMake(unit.position.x + 1 , unit.position.y)];
                }
...
-(void)spawnBombers{
    if (playermoney < 300 || [self.currUnits count] > 14 || bomberlevel > 1.5)
    {
        //Insufficient funds
    }
    else
    {
        bomberlevel = bomberlevel + 0.1;
            ...
            if ([self.enemyUnits count] > 0) {
                for (Units *buffer in self.enemyUnits){
                    if (CGRectIntersectsRect(barriers.frame , buffer.frame)) {
                        ...
                        [barriers setHealth:(barriers.health - buffer. damage)];
                        [buffer setHealth:(buffer.health - barriers.damage)];
            ...
            if ([self.enemyUnits count] > 0) {
                for (Units *buffer in self.enemyUnits){
                    if (CGRectIntersectsRect(sword.frame , buffer.frame)) {
                        ...
                        [sword setHealth:(sword.health - buffer.damage)];
                        [buffer setHealth:(buffer.health - sword.damage)];
        ...
        if (sword.type == 3) {
            if ([self.currUnits count] > 0) {
                ...
                if (CGRectIntersectsRect(sword.frame , buffer.frame)) {
                    ...
                    [sword setHealth:(sword.health - 1)];
                }
                }
                for (Units *buffer in self.enemyUnits) {
                    if (CGRectIntersectsRect(sword.frame , buffer.frame)) {
                        ...
                        [self unitBounceBack:sword];
                        [self unitBounceBack:buffer];
                        [buffer setHealth:(buffer.health - 5)];
                        [sword setHealth:0];
                    }
                }
            }
...
-(void)AIMove {
    if (AImoney > 500) {
        if ([self.enemyUnits count] < 15) {
            int generate = arc4random() % 8;
            switch (generate) {
                case 0: {
                    Units *barrier = [Units spriteNodeWithImageNamed:@"cavernwarsshield"];
                    [barrier setDamage:(barrierlevel * 1.99995)];
                    [barrier setHealth:((barrierlevel / 2.15) + 210)];
                    [barrier setOwner:1];
                    [barrier setUnitspeed:1];
                ...
                case 1: {
                    Units *saber = [Units spriteNodeWithImageNamed:@"cavernwarssabre"];
                    [saber setDamage:(saberlevel * 3.85)];
                ...
                case 3: {
                    ...
                    [bombers setHealth:1 - bomberlevel / 1.5];
                    [bombers setOwner:1];
                    [bombers setUnitspeed:(1.4 + (bomberlevel / 10))];
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    for (Units *unit in self.currUnits) {
        if (unit.health <= 0) {
            ...
            [toDelete addObject:unit];
            AImoney = AImoney + 400;
        }
    }
    [self.currUnits removeObjectsInArray:toDelete];
    NSMutableArray *anotherToDelete = [NSMutableArray array];
    for (Units *unit in self.enemyUnits) {
        if (unit.health <= 0) {
            [unit removeFromParent];
            //[self.enemyUnits removeObject:unit];
            [anotherToDelete addObject:unit];
            playermoney = playermoney + 500;
        }
    }
    [self.enemyUnits removeObjectsInArray:anotherToDelete];
}
